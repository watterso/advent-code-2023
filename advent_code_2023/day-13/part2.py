    ...
    def _get_smudge(
        ...
        for row in specific_list:
            ...
            for s in wa:
                symmmetry_counter[s] += 1
        summary_dict = {
            key.index_of_char_before_reflection: val
            for key, val in symmmetry_counter.items()
        }
        # print(f"{len(specific_list)-1} | {summary_dict}")
        for key, value in symmmetry_counter.items():
            if value == len(specific_list) - 1:
                return key

    def get_hot_dog_symmetry(self) -> typing.Optional[SymmetryDetails]:
        return self._get_smudge(self.by_rows)

    def get_hamburger_symmetry(self) -> typing.Optional[SymmetryDetails]:
        return self._get_smudge(self.by_columns)
